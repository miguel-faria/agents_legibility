...
class GreedyPrey(Agent):
	...
	def act(self, env) -> int:
		...
		prey_pos = env.agents[self.agent_id].pos
		agents_dist = [math.sqrt((prey_pos[0] - env.agents[h_id].pos[0]) ** 2 + (prey_pos[1] - env.agents[h_id].pos[1]) ** 2)  for h_id in env.hunter_ids]
		...
		for direction in ActionDirection:
			nxt_pos = (max(min(prey_pos[0] + direction.value[0], rows), 0), max(min(prey_pos[1] + direction.value[1], rows), 0))
