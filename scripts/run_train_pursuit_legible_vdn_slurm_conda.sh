...
if [ "$HOSTNAME" = "artemis" ] || [ "$HOSTNAME" = "poseidon" ] || [ "$HOSTNAME" = "maia" ]  ; then
  python "$script_path"/run_train_pursuit_legible_vdn_dqn.py --field-len 10 --hunters 2 --catch-reward 5 --prey-type random --batch-size 64 --buffer-size 10000 --iterations 75000 --episode-steps 800 --warmup 800 --limits 1 7 --eps-type linear --eps-decay 0.1 --start-eps 0.75 --legible-reward q_vals --beta 0.9 --legibility-temp 0.1 --online-lr 0.0005 --train-thresh 0.96 --use-lower-curriculum --logs-dir /mnt/scratch-artemis/miguelfaria/logs/pursuit --models-dir /mnt/data-artemis/miguelfaria/deep_rl/models --data-dir /mnt/data-artemis/miguelfaria/deep_rl/data
